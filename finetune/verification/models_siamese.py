...
class VisionTransformer(nn.Module):
    def __init__(self, 
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, 2001 + num_prefix_tokens + num_prompt_tokens, embed_dim))
    ...
    def forward(self, x, 
                ...
                return_weighted_sum_all_layers=False, 
        ...
        elif return_weighted_sum_all_layers:
...
class SiameseViT(nn.Module):
    ...
    def forward(self, x, 
        ...
        x1, x2 = x[:, 0, :, :], x[:, 1, :, :]
        # Get representations from VisionTransformer
        rep1 = self.model(x1, return_all_layers=return_all_layers, return_cls_representation=return_cls_representation, return_weighted_sum_all_layers=return_weighted_sum_all_layers, return_cls_representation_weightes_sum=return_cls_representation_weightes_sum, return_knowledge_ensembler=False)
        rep2 = self.model(x2, return_all_layers=return_all_layers, return_cls_representation=return_cls_representation, return_weighted_sum_all_layers=return_weighted_sum_all_layers, return_cls_representation_weightes_sum=return_cls_representation_weightes_sum, return_knowledge_ensembler=False)

        if return_all_layers or return_cls_representation or return_weighted_sum_all_layers or return_cls_representation_weightes_sum or return_knowledge_ensembler:
            return rep1, rep2

        # Combine and pass through final layers for the prediction task
        x_combined = torch.cat((rep1, rep2), dim=1)
